...
class InferenceModule:
    ...
    def getObservationProb(self, noisyDistance, pacmanPosition, ghostPosition, jailPosition):
        ...
        "*** YOUR CODE HERE ***"
        # Assume getObservationProb works properly
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        possibleGhostPos = self.allPositions # All possible ghost pos, including jail
        allPossiblePos = util.Counter() # Make a place to store all possible pos for ghosts
        pacmanPos = gameState.getPacmanPosition() # Get Pacman's pos
        jailPos = self.getJailPosition()

        for ghostPos in possibleGhostPos : # Look at each ghost position
            # Calculate observation prob - obs for ghost, pacman pos, ghost pos, and jail pos for ghost
            # P(obs | pacmanPos, ghostPos, jailPos)
            em = self.getObservationProb(observation, pacmanPos, ghostPos, jailPos) # Emission model helper

            ghostBelief = self.beliefs[ghostPos] # Look at prob of a ghost being at that pos
            allPossiblePos[ghostPos] = em * ghostBelief # Update

        self.beliefs = allPossiblePos # Update self.beliefs
        self.beliefs.normalize() # Given
    ...
    def elapseTime(self, gameState):
        ...
        allPossiblePos = util.Counter() # Placeholder for new self.beliefs
        allPos = self.allPositions # All positions

        for oldPos in allPos : # Iterate over all positions
            newDist = self.getPositionDistribution(gameState, oldPos) # Get dist over new ghost pos

            for newPos, prob in newDist.items() : # Iterate over all possible ghost pos in dist
                # Update placeholder new.beliefs
                allPossiblePos[newPos] = allPossiblePos[newPos] + (self.beliefs[oldPos] * prob)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        self.particles = [] # Given
        "*** YOUR CODE HERE ***"
        count = 0 # Initialize count

        while count < self.numParticles : # Iterate while count less than numParticles

            for pos in self.legalPositions : # Iterate over each legal position

                if count < self.numParticles : # If count less than numParticles

                    self.particles.append(pos) # Append to self.particles
    ...
    def observeUpdate(self, observation, gameState):
        ...
        tmp = DiscreteDistribution() # Given
        pacmanPos = gameState.getPacmanPosition() # Get Pacman position
        jailPos = self.getJailPosition() # Get jail position
        wgts = {} # Store weights

        for particle in self.particles : # Iterate through each particle
            if particle not in wgts : # If weight not already calculated
                wgt = self.getObservationProb(observation, pacmanPos, particle, jailPos) # Calculate weight
                wgts[particle] = wgt # Store in wgts

            tmp[particle] += wgts[particle] # Add to dist
        
        if tmp.total() == 0 : # Special case
            self.initializeUniformly(gameState) # Reinitialize list of particles
            return

        newPart = [tmp.sample() for _ in range(self.numParticles)] # Sample from tmp weighted distribution
        self.particles = newPart # Assign new list of particles
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        tmp = [] # List of particles

        for ind, oldPos in enumerate(self.particles): # Iterate for each particle
            newPosDist = self.getPositionDistribution(gameState, oldPos) # Based on current state and gameState
            smp = newPosDist.sample() # Sample next state
            tmp.append(smp) # Append to list

        self.particles = tmp # Assign new list
    ...
    def getBeliefDistribution(self):
        ...
        beliefs = util.Counter() # Make a counter
        
        for particle in self.particles : # Iterate over each particle
            beliefs[particle] += 1.0 # Add 1
        
        beliefs.normalize() # Normalize dist
        return beliefs
