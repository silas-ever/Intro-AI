...
class Perceptron(object):
    ...
    def sigmoid(self, value):
        ...
        denom = (1 + exp(-1 * value)) # Calculate the denominator
        out = 1 / denom # Reciprocal
        return out # Return
    ...
    def sigmoidActivation(self, inActs):
        ...
        inActs = [1.0] + inActs # Add 1 for bias input

        sig = 0 # Initialize sum
        sig = self.getWeightedSum(inActs) # Get weighted sum of input
        
        activationValue = self.sigmoid(sig) # Calculate sigmoid
        return activationValue # Return
    ...
    def sigmoidDeriv(self, value):
        ...
        sigmoid = self.sigmoid(value) # Calculate sigmoid of input
        deriv = (1 - sigmoid) * sigmoid # Calculate derivative of sigmoid
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        sigmoidAct = self.sigmoidActivation(inActs) # Calculate activation value
        deriv = (1 - sigmoidAct) * sigmoidAct # Calculate derivative of activation
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        acts = [1.0] + inActs # Add 1 for bias input
        ind = 0 # Initialize index

        for weight in self.weights: # Iterate over all weights

            modification = alpha * acts[ind] * delta # Calculate modification
            self.weights[ind] = weight + modification # Update weight

            totalModification += abs(modification) # Add to total modification
            ind += 1 # Increment index

        return totalModification # Return
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        outList = [inActs] # Initialize output list
        acts = inActs[:] # Initialize acts

        for layer in self.layers : # Iterate over each later
            newActs = [] # Initialize new acts

            for neuron in layer : # Iterate over each node in current layer
                newActs.append(neuron.sigmoidActivation(acts)) # Append to new acts list
            
            acts = newActs # Update acts
            outList.append(newActs) # Append new acts to output list

        return outList # Return
    ...
    def backPropLearning(self, examples, alpha):
        ...
        for example in examples : # for each example
            ...
            for outputNum in range(len(example[1])) :
                ...
                error = example[1][outputNum] - lastLayerOutput[outputNum] # Calculate error using example and layer output
                delta = error * gPrime # Calculate delta 
            ...
            for layerNum in range(self.numHiddenLayers - 1, -1, -1) :
                ...
                for neuronNum in range(len(layer)):
                    gPrime = layer[neuronNum].sigmoidActivationDeriv(allLayerOutput[layerNum]) # Get deriv of each layer
                    nextErr = 0 # Initializing next layer's error
                    for nextNeur in range(len(nextLayer)) : # Iterate over each num in next layer of neurons
                        delt = deltas[0][nextNeur] # Get delta
                        nextWeights = nextLayer[nextNeur].weights[neuronNum + 1] # Get next weights from layer
                        weightedDel = nextWeights * delt # Calculate weighted deltas
                        nextErr = nextErr + weightedDel # Calculate next layer error
                    delta = gPrime * nextErr # Delta for this neuron - Carefully look at the equation here, it is easy to do this by intuition incorrectly
            ...
            for numLayer in range(0, self.numLayers) :
                ...
                for numNeuron in range(len(layer)) :
                    ourDelta = deltas[numLayer][numNeuron] # Get delta at layer and neuron
                    layerOut = allLayerOutput[numLayer] # Get output at particular layer
                    weightMod = layer[numNeuron].updateWeights(layerOut, alpha, ourDelta) # Update weights accordingly
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    iteration = 0
    trainError = 0
    weightMod = float('inf') # Initializing weight modification so the loop will execute

    """
    Iterate for as long as it takes to reach weight modification threshold
    """
    while (iteration < maxItr) and (weightMod > weightChangeThreshold) : # While under maxItr and weightModif is higher
        trainingError, weightMod = nnet.backPropLearning(examplesTrain, alpha) # Back propogation
    ...
    for ex in examplesTest : # For each text example
        result = nnet.feedForward(ex[0]) # Feed forward to get NN result
        roundList = [round(value) for value in result[-1]] # Round list of outputs from output layer

        if roundList == ex[1]: # If matches
            testCorrect += 1 # Add to testCorrect
        else:
            testError += 1 # Add to testError
            
    testAccuracy = testCorrect / len(examplesTest) # Calculate accuracy of neural net
    ...
    """return something"""
    return nnet, testAccuracy # Return